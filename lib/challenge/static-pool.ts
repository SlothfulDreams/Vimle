 ...
 */
export function getPoolSize(): { total: number; byDifficulty: Record<DifficultyLevel, number> } {
  const byDifficulty = {
 ...
 */
export function getRandomChallengeFromPool(difficulty: DifficultyLevel): StaticChallenge {
  const pool = STATIC_CHALLENGE_POOL[difficulty];
...
export function validateChallengePool(): {
  ...
  for (const [difficulty, challenges] of Object.entries(STATIC_CHALLENGE_POOL)) {
    ...
    challenges.forEach((challenge, index) => {
      ...
      if (challenge.content && challenge.content.length < 10) {
        warnings.push(`${prefix}: Content is very short (${challenge.content.length} chars)`);
      }

      if (challenge.content && challenge.content.length > 1000) {
        warnings.push(`${prefix}: Content is very long (${challenge.content.length} chars)`);
      }

      // Basic syntax check
      if (challenge.content && !challenge.content.includes('{') && !challenge.content.includes('=')) {
        warnings.push(`${prefix}: Content may not be valid JavaScript/TypeScript`);
      }
    });
  }
