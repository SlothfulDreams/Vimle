...
export function validateGeneratedCode(
  ...
  if (!content || content.trim().length < VALIDATION_RULES.minLength) {
    errors.push(`Content must be at least ${VALIDATION_RULES.minLength} characters`);
  }

  if (content.length > VALIDATION_RULES.maxLength) {
    errors.push(`Content must be less than ${VALIDATION_RULES.maxLength} characters`);
  }

  // Line count validation
  const lines = content.split('\n');
  if (lines.length > VALIDATION_RULES.maxLines) {
    errors.push(`Content must have fewer than ${VALIDATION_RULES.maxLines} lines`);
  }
  ...
  if (difficulty && ["medium", "hard"].includes(difficulty)) {
    ...
    if (nonEmptyLines.length < VALIDATION_RULES.minLinesForComplex) {
      warnings.push(`${difficulty} challenges should have at least ${VALIDATION_RULES.minLinesForComplex} lines of code`);
    }
  }
  ...
  const hasValidStructure = VALIDATION_RULES.requiredPatterns.hasCodeStructure.some(
    pattern => pattern.test(content)
  );

  if (!hasValidStructure) {
    errors.push("Content doesn't appear to contain valid JavaScript/TypeScript code structures");
  }
...
export function validateVimPracticeContent(content: string): ValidationResult {
  ...
  if (charVariety < 5) {
    warnings.push("Content has low character variety, may not provide good vim practice");
  }

  // Should have some indentation for vim practice
  const hasIndentation = /^\s+/m.test(content);
  if (!hasIndentation) {
    warnings.push("Content lacks indentation, missing opportunity for vim practice");
  }
  ...
  if (opportunityCount < 2) {
    warnings.push("Content may not provide enough variety for comprehensive vim practice");
  }
