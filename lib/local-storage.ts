 ...
 */
export function getLocalAttemptForDate(date: string): LocalChallengeAttempt | null {
...
export function saveLocalCompletion(
  ...
  const existingIndex = attempts.findIndex(attempt => attempt.challengeDate === challengeDate);
  ...
  const filteredAttempts = attempts.filter(attempt => attempt.challengeDate >= cutoffDate);
...
export function getLocalStats() {
  ...
  const totalTime = completedAttempts.reduce((sum, attempt) => sum + attempt.timeMs, 0);
  const averageTime = totalTime / completedAttempts.length;
  const bestTime = Math.min(...completedAttempts.map(attempt => attempt.timeMs));
  
  // Calculate current streak
  let currentStreak = 0;
  const sortedAttempts = [...completedAttempts].sort((a, b) => b.challengeDate.localeCompare(a.challengeDate));
